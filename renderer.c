...
void initShaders(Renderer *renderer) {
    char *simpleVertex = readFile("./shaders/simple.vert");
    char *simpleFragment = readFile("./shaders/simple.frag");

    renderer->shaders[0].shaderID = linkProgram(simpleVertex, simpleFragment);
    renderer->shaders[0].name = "simple";
...
void drawRectangle(Renderer *renderer, Vec2f position, Vec2f size, Color color) {
    ...
    drawTriangle(renderer, p1, color, uv1, p3, color, uv3, p2, color, uv2);
    drawTriangle(renderer, p2, color, uv2, p3, color, uv3, p4, color, uv4);
}
